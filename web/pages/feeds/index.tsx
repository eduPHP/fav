...
export default function Home() {
  ...
  const handleDeleteRss = useCallback(async (id: number) => {
    showDialog({
      title: 'Delete RSS Feed',
      description: 'If you choose to continue, this action will be permanent.',
      type: 'danger',
      options: {
        confirm: 'Delete',
      },
  ...
  const handleYOLO = useCallback(async (type: 'success' | 'danger' | 'info') => {
    showDialog({
      title: 'Delete RSS Feed',
      description: 'If you choose to continue, this action will be permanent.',
      type,
    ...
    <>
      ...
      <div>
        <Link href={'/feeds/create'}>
          <a className="rounded bg-blue-400 px-6 py-2 text-blue-900">Create</a>
        </Link>
        <ul className="mt-6 divide-y divide-gray-500 rounded-lg shadow bg-gray-600">
          ...
          {feeds.map(feed => (
            <li className="py-6 px-8 flex items-center justify-between" key={feed.id}>
              <span className="flex-1 max-w-[80%]">
                <span className="text-indigo-300 block mb-2 text-xl">{feed.name}</span>
                <span className="flex items-center">
                  ...
                  <span className=" table-cell overflow-hidden">{feed.url}</span>
                </span>
              </span>
              <div className="flex flex-col justify-between items-end">
                ...
                <button onClick={() => handleDeleteRss(feed.id)} className="text-indigo-300 hover:underline" type="button">
                  Delete
                </button>
              </div>
            </li>
