  ...
  return (
    <div className={
      ...
      <div className="flex-1">
        ...
        {toast.description && <p className="mt-1 text-sm leading-6 opacity-80 border-0 bg-transparent"
                                 dangerouslySetInnerHTML={{__html: toast.description}}
        />}
      </div>
      <button className="absolute right-3 top-5 opacity-60" type="button" onClick={() => removeToast(toast.id)}>
        <FiXCircle size={18} />
      </button>
    </div>
  );
