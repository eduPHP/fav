...
class MemCache implements CacheControlInterface {
  async put(key: string, content: object, duration: number = 1): Promise<void> {
    memoryCache.put(`__cache${key.replace(/[\/\\]/g, '-')}`, content, duration * 1000 * 60);
    console.log(`Cache was wrote for '__cache${key.replace(/[\/\\]/g, '-')}'`)
  }

  async get(key: string, def: object | null = null): Promise<string | object | null> {
...
class FileCache implements CacheControlInterface {
  ...
  async get(key: string, def: object | null = null): Promise<object | null> {
    const fileName = path.join(this.folder, `__cache${key.replace(/[\/\\]/g, '-')}`);
